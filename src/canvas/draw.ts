...
import { ColAlignType, FontSetting, RowAlignType } from '../core';

interface DrawTextAttrType {
  align: RowAlignType;
  valign: ColAlignType;
  color: string;
  strike: boolean;
...
class DrawBox {
  ...
  padding: number
  bgcolor: string
  borderTop: any // fixme: use type guard
  borderRight: any
  borderBottom: any
  borderLeft: any
  ...
  setBorders({ top, bottom, left, right }: { top: number; bottom: number; left: number; right: number }) {
  ...
  texty(align: ColAlignType, h: number) {
...
type FontLineType = 'underline' | 'strike'
...
class Draw {
  ...
  */
  text(mtxt: string, box: DrawBox, attr: DrawTextAttrType, textWrap = true) {
    const { ctx } = this;
    const {
  ...
  strokeBorders(box: DrawBox) {
    ...
    if (borderTop) {
      (this.border as any)(...borderTop);
      // console.log('box.topxys:', box.topxys());
      this.line(...box.topxys());
    }
    if (borderRight) {
      (this.border as any)(...borderRight);
      this.line(...box.rightxys());
    }
    if (borderBottom) {
      (this.border as any)(...borderBottom);
      this.line(...box.bottomxys());
    }
    if (borderLeft) {
      (this.border as any)(...borderLeft);
