...
const evalSubExpr = (subExpr: any, cellRender: any) => {
  const [fl] = subExpr;
  let expr = subExpr;
  if (fl === '"') {
...
// formulaMap: {'SUM': {}, ...}
// cellRender: (x, y) => {}
const evalSuffixExpr = (srcStack: any, formulaMap: any, cellRender: any, cellList: any) => {
  const stack: any[] = [];
...
const cellRender = (src: string, formulaMap: any, getCellText: any, cellList = []) => {
  if (src[0] === '=') {
    ...
    return evalSuffixExpr(
      stack,
      formulaMap,
      (x: any, y: any) => cellRender(getCellText(x, y), formulaMap, getCellText, cellList),
      cellList,
    );
  }
  return src;
