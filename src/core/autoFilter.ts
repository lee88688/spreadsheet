...
class Filter {
  ...
  constructor(ci: number, operator: string, value: any) {
    this.ci = ci;
...
export default class AutoFilter {
  ...
  setData({ ref, filters, sort }: { ref: any; filters: any[]; sort: any }) {
    if (ref != null) {
      this.ref = ref;
  ...
  addFilter(ci: number, operator: string, value: any) {
  ...
  setSort(ci: number, order: string) {
  ...
  filteredRows(getCell: any) {
    // const ary = [];
    // let lastri = 0;
    const rset = new Set<any>();
    const fset = new Set<any>();
    if (this.active()) {
