...
import { CopyCellType, DeleteCellType, Rows } from './row';
...
export default class DataProxy {
  ...
  change: Function
  hyperlinks: object // todo: find usage
  comments: object // todo: find usage

  constructor(name: string, settings: SheetSetting) {
    this.settings = helper.merge(defaultSettings, settings || {}) as SheetSetting;
    // save data begin
  ...
  private canPaste(src: CellRange, dst: CellRange, error: (em: string) => void = () => undefined) {
    const { merges } = this;
  ...
  private copyPaste(srcCellRange: CellRange, dstCellRange: CellRange, what: CopyCellType, autofill = false) {
  ...
  private setStyleBorder(ri: number, ci: number, bss: { top?: any; bottom?: any; left?: any; right?: any }) {
    const { styles, rows } = this;
    const cell = rows.getCellOrNew(ri, ci);
    let cstyle: StyleSetting = {} as StyleSetting;
    if (cell.style !== undefined) {
      cstyle = helper.cloneDeep(styles[cell.style]);
    }
    cstyle = helper.merge(cstyle, { border: bss }) as StyleSetting;
    cell.style = this.addStyle(cstyle);
  }

  private setStyleBorders({ mode, style, color }: any) {
    ...
    if (mode === 'outside' && !multiple) {
      ...
      selector.range.each((ri, ci) => {
        ...
        if (cell && cell.style !== undefined) {
          const ns: any = helper.cloneDeep(styles[cell.style]);
          delete ns.border;
  ...
  paste(what: CopyCellType = 'all', error = () => undefined) {
    // console.log('sIndexes:', sIndexes);
    const { clipboard, selector } = this;
    if (clipboard.isClear()) return false;
    if (!this.canPaste(clipboard.range as CellRange, selector.range, error)) return false;

    this.changeData(() => {
      if (clipboard.isCopy()) {
        this.copyPaste(clipboard.range as CellRange, selector.range, what);
      } else if (clipboard.isCut()) {
        this.cutPaste(clipboard.range as CellRange, selector.range);
      }
    });
    return true;
  }
  ...
  autofill(cellRange: CellRange, what: CopyCellType, error = () => undefined) {
  ...
  setSelectedCellAttr(property: string, value: any) {
    this.changeData(() => {
  ...
  setSelectedCellText(text: string, state: 'input' | 'finished' = 'input') {
    ...
    if (autoFilter.active()) {
      ...
      if (filter) {
        const vIndex = filter.value.findIndex((v: string) => v === oldText);
        if (vIndex >= 0) {
  ...
  xyInSelectedRect(x: number, y: number) {
    ...
    return x1 > left && x1 < (left + (width ?? 0))
      && y1 > top && y1 < (top + (height ?? 0));
  }
  ...
  getClipboardRect() {
    ...
    if (!clipboard.isClear()) {
      return this.getRect(clipboard.range as CellRange);
    }
  ...
  setAutoFilter(ci: number, order: string, operator: string, value: any) {
  ...
  resetAutoFilter() {
    ...
    const { rset, fset } = autoFilter.filteredRows((r: number, c: number) => rows.getCell(r, c));
    const fary = Array.from(fset) as number[];
  ...
  deleteCell(what: DeleteCellType = 'all') {
    const { selector } = this;
    this.changeData(() => {
  ...
  delete(type: 'row' | 'column') {
    this.changeData(() => {
      ...
      merges.shift(type, si, -size, (ri, ci, rn, cn) => {
        ...
        const cell = rows.getCell(ri, ci);
        if (!cell?.merge) return; // fixme: use type guards
        cell.merge[0] += rn;
        cell.merge[1] += cn;
  ...
  eachMergesInView(viewRange: CellRange, cb: (cr: CellRange) => void) {
  ...
  unhideRowsOrCols(type: 'row' | 'col', index: number) {
    this[`${type}s` as ('rows' | 'cols')].unhide(index);
  }

  rowEach(min: number, max: number, cb: (i: number, y: number, rowHeight: number) => void) {
    let y = 0;
    const { rows } = this;
    const frset = this.exceptRowSet;
    const frary = [...frset];
  ...
  colEach(min: number, max: number, cb: (i: number, x: number, colWidth: number) => void) {
    let x = 0;
    const { cols } = this;
  ...
  setData(d: any) {
    Object.keys(d).forEach((property) => {
      if (property === 'merges' || property === 'rows'
        ...
        (this as any)[property] = d[property];
      }
    });
    return this;
  }
